...
final class CameraViewModel {
    ...
    private(set) var properties: [UInt16: [String: Any]] = [:]
    ...
    private init() {
        NotificationCenter.default.addObserver(forName: NSNotification.Name(SonyCameraBridgeStateDidChangeNotification), object: bridge, queue: .main) { [weak self] note in
            guard let self else { return }
            self.properties = self.bridge.stateSnapshot() as? [UInt16: [String: Any]] ?? [:]
